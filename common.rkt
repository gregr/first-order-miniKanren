...
(define (reify tm st)
  ...
  (let ((results (let loop ((tm tm) (st st))
    ...
    (let* ((walked-sub (walk* tm results))
           ...
           (diseq (sort diseq (lambda (x y) (< (car x) (car y)))))
           (diseq (map (lambda (=/=) (cdr =/=)) diseq))
...
(define (term<? u v)
  (cond
    ...
    ((eqv? u #t) v)
    ((eqv? v #t) #f)
    ((number? u) (or (not (number? v)) (< u v)))
    ((number? v) #f)
    ((symbol? u) (or (not (symbol? v)) (symbol<? u v)))
    ((symbol? v) #f)
    ((string? u) (or (not (string? v)) (string<? u v)))
    ((string? v) #f)
    ((pair? u) (or (not (pair? v)) (or (term<? (car u) (car v)) 
                                       (and (equal? (cdr u) (cdr v)) 
                                            (term<? (cdr u) (cdr v))))))
    ((pair? v) #f)
    (else #f)
  ))
